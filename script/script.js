...
function loadRanking(name) {
    ...
    $('#inside_ranking_page thead tr th:eq(2)').text(tgt_rank.value_name);

    for (var i = 0; i < 5; i++) {
        var pos = i + 1;
        var tgt_inside_rank = inside_rankings({ranking_id: tgt_rank.id, position: pos}).first();
        $('#inside_ranking_page tbody tr:eq(' + i + ') td:eq(0)').text(tgt_inside_rank.position);
        $('#inside_ranking_page tbody tr:eq(' + i + ') td:eq(1)').text(tgt_inside_rank.nome);
        $('#inside_ranking_page tbody tr:eq(' + i + ') td:eq(2)').text(tgt_inside_rank.value);
    }
